...
- [x] T3. `controller_test.go` `TestController_OfferedCurrentChannel_PacesWrites` — log replay:
      10 at t0 sent; 11..18 every 5 s stored; +44 s still one call; +45 s one send with 18;
      +50 s 19 stored; +90 s 19 sent; three `UpdateDynamicCurrent` in total. Red: unexpected
...
- [x] T4. `…_SendsAfterTheWindow` — set at t0, +20 s a different value goes out synchronously
      (the `>=` boundary), +39 s stored, +65 s sent. Red: 3 calls where 2 were expected - the
      +39 s write went out
- [x] T5. `TestController_StopChargepointCharging_StampsTheChannel` — stop at t0 sent; +1 s set 10
      stored; +44 s sent. Red: `Should not have called with given arguments` -
...
- [x] A. Implement: `chargerCommand`, `pace`/`sentAt`/`pending`, `dispatch`, `send`,
      ...
      a stop; `OfferedCurrentDeferralTime()` (`offered_current_deferral_time`, 45 s) plus the
      packaged default; client throttle, `force` and `cfgSrv` removed from `api`; callers and
...
- [x] T6. `TestController_StopChargepointCharging_IsReplacedByAStart` — set 10 at t0; +5 s stop
      stored; +5 s start returns nil; +35 s `UpdateDynamicCurrent(16)` once, `StopCharging`
...
- [x] T7. `TestController_SetChargepointPhaseMode_DefersTheResume` (replaces
      ...
      `UpdateDynamicCurrent` not called synchronously; +45 s `UpdateDynamicCurrent(16)` once and
      `SetRequestedOfferedCurrent(16, t0+45s)`. Red: `phase mode set to 1, but the charger did not
      resume at 16A`
...
- [x] T10. `TestController_StartChargepointCharging_DeferredStartReportsSuccess` — set 10 at t0;
      +5 s start returns nil without a synchronous echo wait; +40 s `UpdateDynamicCurrent(16)`
...
- [x] 3. `make test` green. Coverage (statements / functions): `internal/easee` 85.3% / 87.0%,
      `internal/api` 86.1% / 93.0%, `internal/config` 96.3% / 100%; the channel itself:
