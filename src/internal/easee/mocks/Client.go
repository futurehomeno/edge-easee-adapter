...
// EnergyPerHour provides a mock function with given fields: chargerID, from, to
func (_m *Client) EnergyPerHour(chargerID string, from time.Time, to time.Time) ([]easee.Measurement, error) {
	ret := _m.Called(chargerID, from, to)

	var r0 []easee.Measurement
	if rf, ok := ret.Get(0).(func(string, time.Time, time.Time) []easee.Measurement); ok {
		r0 = rf(chargerID, from, to)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]easee.Measurement)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(string, time.Time, time.Time) error); ok {
		r1 = rf(chargerID, from, to)
	} else {
		r1 = ret.Error(1)
	}
